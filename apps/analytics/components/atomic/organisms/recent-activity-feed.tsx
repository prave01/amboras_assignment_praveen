...
export function RecentActivityFeed({ events, isLoading, hasError, timeTick }: RecentActivityFeedProps) {
  return (
        ...
        {isLoading ? (
          ...
          <div className="max-h-115 space-y-3 overflow-auto pr-1">
            {events.length === 0 ? (
              ...
              events.map((event) => (
                <article key={event.eventId ?? `${event.timestamp}-${event.eventType}-${timeTick}`} className="rounded-xl border border-border/70 bg-white/70 p-3">
